package main

import (
	"fmt"
	"os"
	"slices"
)

type Pattern []int16

func StringToPattern(s string) Pattern {
	p := make(Pattern, len(s))
	for i := 0; i < len(s); i++ {
		p[i] = int16(s[i])
	}
	return p
}

func patch(in []byte, find Pattern, replace []byte) []byte {
	for i := 0; i < len(in)-len(find); i++ {
		cmp := in[i : i+len(find)]
		if slices.EqualFunc(cmp, find, func(b byte, i int16) bool {
			if i == -1 {
				return true
			}

			return int16(b) == i
		}) {
			fmt.Printf("Found pattern at offset %v\n", i)
			for j := i; j < i+len(replace); j++ {
				in[j] = replace[j-i]
				fmt.Println(replace[j-i])
			}
		}
	}

	return in
}

var (
	PortalPattern            = StringToPattern(".actual.battle.net")
	ConnectToModulusPattern  = Pattern{0x91, 0xD5, 0x9B, 0xB7, 0xD4, 0xE1, 0x83, 0xA5}
	CryptoEdPublicKeyPattern = Pattern{0x15, 0xD6, 0x18, 0xBD, 0x7D, 0xB5, 0x77, 0xBD}

	RsaModulus = []byte{}
	CryptoEdPublicKey = []byte{0x02, 0x59, 0x6F, 0x0D, 0x0C, 0x06, 0x1A, 0x8B, 0x30, 0x74, 0x59, 0x88, 0xFD, 0x72, 0xC5, 0x9E,
		0x29, 0xEC, 0x36, 0x7F, 0xB0, 0xF3, 0x41, 0xF2, 0x8E, 0x0F, 0x08, 0xD0, 0x37, 0xBA, 0xFC, 0x69}
	PortalReplace = make([]byte, len(PortalPattern))
)

func main() {
	src := os.Args[1]

	data, err := os.ReadFile(src)
	if err != nil {
		panic(err)
	}

	data = patch(data, PortalPattern, PortalReplace)
	data = patch(data, ConnectToModulusPattern, RsaModulus)
	data = patch(data, CryptoEdPublicKeyPattern, CryptoEdPublicKey)

	os.Remove("Arctium")
	os.WriteFile("Arctium", data, 0777)
}
